...
test_that("When the button is clicked, the options update", {
  local_mocked_bindings(
    updateSelectInput = create_test_update_fn("updateSelectInput"), 
    .package = "shiny" 
  )
...
If there is more than one function required to create mocked bindings for, then `create_test_update_fns` can be used. This creates a named list of the required functions.

```{r create_multi_example}
# Simple module where when a button is clicked, a select input is updated to select "h" out of the letters
...
## Shiny Package Helpers

For the shiny package specifically, the function `use_shiny_testers` is available as a blanket assignment of all update functions in the shiny package.
